...
public interface IJobApplicationDao extends JpaRepository<JobApplication, Long> {
	...
	@Query("select new com.jobportal.dto.JobApplicationsListDTO(j.id, j.job.id, j.job.jobTitle, j.coverLetter, j.freelancer.id, CONCAT(j.freelancer.firstName,' ', j.freelancer.lastName), j.freelancer.userName)  from JobApplication j, BookmarkedFreelancer bf where j.job.id=:jobId and j.freelancer.id = bf.freelancer.id order by j.id")
	List<JobApplicationsListDTO> findAllByJobId(@Param("jobId") Long jobId);
	
	@Query("select new com.jobportal.dto.JobApplicationsListDTO(j.id, j.job.id, j.job.jobTitle, j.coverLetter, f.id, CONCAT(f.firstName,' ', f.lastName), f.userName)  from JobApplication j, Freelancer f where j.freelancer = f order by j.id")
	List<JobApplicationsListDTO> findAllApps();
	
	@Query("select new com.jobportal.dto.JobApplicationsListDTO(j.id, j.job.id, j.job.jobTitle, j.coverLetter, f.id, CONCAT(f.firstName,' ', f.lastName), f.userName)  from JobApplication j, Freelancer f where f.id=:frId and j.job.id=:jobId")
